export function pawnRules(
  ...
  if (
    pawnCaptureRules(previousX, previousY, newX, newY, boardState, colorOfPiece)
  ) {
    return true;
  }
...
export function whitePawnRules(
  previousX,
  previousY,
  newX,
  newY,
...
export function blackPawnRules(
  previousX,
  previousY,
  newX,
  newY,
...
export function pawnCaptureRules(
  previousX,
  previousY,
  newX,
  newY,
  ...
  colorOfPiece === 'White'
    ? (AllowedYAxisToMove = previousY + 1)
    : (AllowedYAxisToMove = previousY - 1);

  if (
    !isThereAPieceInThePawnCaptureSquare(newX, newY, boardState, colorOfPiece)
  ) {
    return false;
  }

  if (
    (newY === AllowedYAxisToMove && newX === previousX - 1) ||
    (newY === AllowedYAxisToMove && newX === previousX + 1)
  ) {
    return true;
  }
...
function isThereAPieceInThePawnCaptureSquare(
  newX,
  newY,
  boardState,
  colorOfPiece
...
function twoFrontalSquaresAreBlocked(
  previousX,
  previousY,
  boardState,
  colorOfPiece
  ...
  if (colorOfPiece === 'White') {
    for (let i = 0; i < boardState.length; i++) {
      if (
        boardState[i].key === `${previousX},${previousY + 1}` ||
        boardState[i].key === `${previousX},${previousY + 2}`
      ) {
        if (boardState[i].props.pieceImg !== undefined) {
          return true;
        }
