...
export function isACastleMovement(boardState, newX, colorOfPiece, typeOfPiece) {
  ...
  if (
    pathToCastleIsOccupiedOrUnderAttack(boardState, shortOrLong, colorOfPiece)
  ) {
    return false;
  }
...
function kingIsInPositionForCastling(boardState, colorOfPiece) {
  for (let i = 0; i < boardState.length; i++) {
    if (
      boardState[i].props.type === 'King' &&
      boardState[i].props.color === colorOfPiece
    ) {
    ...
    if (colorOfPiece === 'Black') {
      for (let i = 0; i < boardState.length; i++) {
        if (boardState[i].key === '7,7') {
          if (
            boardState[i].props.type === 'Rook' &&
            boardState[i].props.color === 'Black'
          ) {
            return true;
          } else {
            return false;
    ...
    if (colorOfPiece === 'White') {
      for (let i = 0; i < boardState.length; i++) {
        if (boardState[i].key === '7,0') {
          if (
            boardState[i].props.type === 'Rook' &&
            boardState[i].props.color === 'White'
          ) {
            return true;
          } else {
            return false;
...
function pathToCastleIsOccupiedOrUnderAttack(
  boardState,
  shortOrLong,
  colorOfPiece
) {
  ...
  for (let i = 0; i < coordinatesToBeChecked.length; i++) {
    const x = coordinatesToBeChecked[i][0];
    const y = coordinatesToBeChecked[i][2];
