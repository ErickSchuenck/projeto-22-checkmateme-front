...
import {
  isACastleMovement,
  typeOfCastle
} from '../../Rulings/isACastleMovement';
...
function InsertStartingPieces() {
  for (let i = 0; i < 2; i++) {
    ...
    initialBoardState.push({
      pieceImg: `assets/${color}Rook.png`,
      XPosition: 0,
      YPosition,
      type: 'Rook',
      color
    ...
    initialBoardState.push({
      pieceImg: `assets/${color}Knight.png`,
      XPosition: 1,
      YPosition,
      type: 'Knight',
      color
    ...
    initialBoardState.push({
      pieceImg: `assets/${color}Bishop.png`,
      XPosition: 2,
      YPosition,
      type: 'Bishop',
      color
    ...
    initialBoardState.push({
      pieceImg: `assets/${color}Queen.png`,
      XPosition: 3,
      YPosition,
      type: 'Queen',
      color
    ...
    initialBoardState.push({
      pieceImg: `assets/${color}King.png`,
      XPosition: 4,
      YPosition,
      type: 'King',
      color
...
export default function ChessBoard() {
  ...
  function grabPiece(event) {
    ...
    if (element.classList.contains('Piece')) {
      ...
      setCoordinateX(
        Math.floor((event.clientX - chessboard.offsetLeft) / _squareSize)
      );
      setCoordinateY(
        Math.abs(
          Math.ceil(
            (event.clientY - chessboard.offsetTop - _squareSize * 8) /
              _squareSize
          )
        )
      );
  ...
  function movePiece(event) {
    ...
    const leftConstraint = chessBoardRef.current.offsetLeft;
    const rightConstraint =
      chessBoardRef.current.offsetLeft + chessBoardRef.current.offsetWidth;
    const topConstraint = chessBoardRef.current.offsetTop;
    const bottomConstraint =
      chessBoardRef.current.offsetTop + chessBoardRef.current.offsetHeight;
  ...
  function dropPiece(event) {
    ...
    if (activePiece) {
      const newX = Math.floor(
        (event.clientX - chessboard.offsetLeft) / squareSize
      );
      const newY = Math.abs(
        Math.ceil(
          (event.clientY - chessboard.offsetTop - 8 * squareSize) / squareSize
        )
      );

      const currentPiece = pieces.find(
        piece =>
          piece.XPosition === coordinateX && piece.YPosition === coordinateY
      );
      if (!currentPiece) return;
      const attackedPiece = pieces.find(
        piece => piece.XPosition === newX && piece.YPosition === newY
      );
      ...
      const castleMovement = isACastleMovement(
        newBoard,
        newX,
        currentPiece.color,
        currentPiece.type
      ...
      const computerMove = sendMoveToServer(
        coordinateX,
        coordinateY,
        newX,
        newY
    ...
    setPieces(pieces => {
      return pieces.map(piece => {
        ...
        if (
          piece.type === 'Rook' &&
          piece.color === color &&
          piece.XPosition === previousRookAxis
        ) {
  ...
  function updateBoard(attackedPiece, newX, newY) {
    setPieces(pieces => {
      return pieces.map(piece => {
        ...
        if (
          piece.XPosition === coordinateX &&
          piece.YPosition === coordinateY
        ) {
